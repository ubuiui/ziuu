...
async def 랭킹(ctx):
    ...
    embed = discord.Embed(title="👑 우리 서버 최고의 자산가 TOP 5 👑", color=discord.Color.gold())
    money_text = ""
    emojis = ["🥇", "🥈", "🥉", "✨", "✨"]
    for idx, (uid, money) in enumerate(sorted_money):
        name = user_names.get(uid, f"유저({uid})")
        money_text += f"{emojis[idx]} **{idx+1}위** - {name} : `{money:,}원`\n"
    embed.description = money_text if money_text else "아직 도박 및 미니게임 참가 기록이 없습니다."
    embed.set_footer(text="💰 도박과 출석체크로 자산을 불려 1등을 차지해 보세요!")
    await ctx.send(embed=embed)
