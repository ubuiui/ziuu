...
async def play_blackjack(ctx, bet):
    uid = ctx.author.id
    if bet > user_money.get(uid, 1000): return await ctx.send(f"⚠️ 자산 부족! 현재 {user_money.get(uid, 1000)}원뿐입니다.")
...
async def 입금(ctx, m: discord.Member, a: int):
    ...
    await ctx.send(f"✅ 관리자 {ctx.author.name}님이 {m.name}에게 {a}원을 지급 완료했습니다. (현재 자산: {user_money[m.id]}원)")

@bot.command()
@commands.has_permissions(administrator=True)
async def 회수(ctx, m: discord.Member, a: int):
    user_money[m.id] = user_money.get(m.id, 1000) - a
    await ctx.send(f"⚠️ 관리자 {ctx.author.name}님이 {m.name}에게서 {a}원을 회수 완료했습니다. (현재 자산: {user_money[m.id]}원)")
...
async def 청소(ctx, n: int):
    await ctx.channel.purge(limit=n + 1); await ctx.send(f"🧹 {n}개 삭제 완료.", delete_after=2)
